...
from pywinauto.controls import common_controls, win32_controls
from utils.tools import get_background_color

from constants import REGEX_CREDIT_HOLD, REGEX_NEGATIVE_ITEM, SYTELINE_WINDOW_TITLE, WHITE, REGEX_RESOLUTION
...
class TransactUnit(Unit, completion_string=TRANSACTION_COMPLETED, status_string=TRANSACTION_STATUS):
	def __init__(self, ID: int):
		try:
			...
			if not self.notes:  # Because HomeGuard -_-
	...
	@classmethod
	def get_units(cls, total_qty: int, qty: int):
...
def transact(app: Application, units, *, debug_mode: bool = False):
	...
	def stage4(unit: Unit) -> bool:
		...
		if any(len(x.parts) > 0 for x in units) or debug_mode:
			...
			sl_win.ApplyFilterButton.wait('ready', wait_duration, wait_interval)
			...
			log.debug(transaction_grid.grid)
			...
			sl_win.IncludeUnpostedButton.click()
			sl_win.ApplyFilterButton.click()
			sl_win.ApplyFilterButton.wait('ready', wait_duration, wait_interval)
		...
		if not debug_mode:
			...
			if unit.passed_QC or not unit.sro_open_status['Operations']:
				...
				try:
					...
					pag.press('esc')
			if unit.passed_QC and unit.sro_open_status['Operations']:
				for x in units:
					x.closed_sros += 1
		sro_operations_time = timer.stop() / len(units)
		for x in units:
	...
	def stage6(unit: Unit) -> bool:
		log.debug("Stage 6 started")
		if unit.passed_QC:
			closed_sros = 0
			rogue_sros = unit.get_rogue_sros()
			if rogue_sros:
				timer = Timer.start()
				log.debug(f"{len(rogue_sros)} Rogue SRO's detected!")
				for r_sro in rogue_sros:
					...
					try:
						...
						pag.press('esc')
					closed_sros += 1
					log.debug(f"Rogue SRO {r_sro['sro']} closed.")
				extra_sro_time = timer.stop() / len(units)
				for x in units:
					x.extra_sro_time += extra_sro_time
					x.closed_sros += closed_sros
				sl_uia.CancelCloseButton.click()
				sl_win.ServiceOrderOperationsButton.wait('visible', wait_duration, wait_interval)
				sleep(2)
		sl_uia.CancelCloseButton.click()
