...
def main():
	...
	app.log_in(usr, pwd)
	print("Loading units...")
	results = mssql.execute("Select distinct [Serial Number] From Operations Where Operation = 'QC' and Convert(date, DateTime) Between '9/1/2017' and '10/1/2017'", fetchall=True)
	for x in results:
	...
	while not q.empty():
		try:
			...
			count = 0
			while (sl_win.UnitEdit.texts()[0].strip() != unit.serial_number_prefix+unit.serial_number) and sl_win.UnitEdit.texts()[0].strip():
				if count >= 30:
