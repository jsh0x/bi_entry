...
try:
	config = Config.open(pathlib.WindowsPath(os.environ["PROGRAMDATA"]) / 'BI_Entry' / 'config.json')
except OSError:
	DefaultConfig()
finally:
	config = Config.open(pathlib.WindowsPath(os.environ["PROGRAMDATA"]) / 'BI_Entry' / 'config.json')
...
__all__ = ['active_time', 'application_filepath', 'username', 'password',
